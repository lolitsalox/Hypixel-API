...
const maps_ = new Map(Object.entries(maps)
  .map(([game, maps]) => [game, new Map(Object.entries(maps).map((([map, {
    name,
  }]) => [map, name])))]));

function getMode(type, mode) {
  const game = modes.find(({
    key,
  }) => key === type);
  if (!game) return mode;
  ...
  return game.modes.find(({
    key,
    keys,
  }) => (keys && keys.includes(mode)) || key === mode) || (subGame && subGame.modes.find(({
    key,
    keys,
